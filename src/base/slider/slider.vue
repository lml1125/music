   ...
   props: {
    ...
    mounted(){
    	...
    	setTimeout(()=>{
    		 ...
    		 this._initDots()  //放在鼠标混动播放的前面，要不点会多两个
    		 this._initSlider();
    ...
    methods:{
    	...
    	_initSlider(){
          ...
          snapSpeed: 400 //轮播图切换的动画时间
    		...
    		this.slider.on('scrollEnd',() => { //滚动结束的时候
    			 let pageIndex = this.slider.getCurrentPage().pageX;//获取滚动的当前页
    			 
    			 if(this.loop){ //自动播放下，会拷贝一份 所以要减一
    			 	 pageIndex -=1
    			 }
    	...
    	_play(){
    		...
    		this.timer = setTimeout( () =>{
    			this.slider.goToPage(pageIndex,0,400) //滚动到对应的页面
    		},this.interval )
    	}
